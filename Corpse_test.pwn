...
public OnGameModeInit()
{
    ...
    SendRconCommand("hostname [eG] LS Apocalyptic 2.1 - [Double XP!] [LagShot]");
...
public OnPlayerConnect(playerid)
{
	...
	format(str, sizeof(str), "» "cred"%s [ID: %d] joined to the server.", GetPName(playerid), playerid);
...
CMD:gototiki(playerid, params[])
{
	...
	if(TikiStatueOn == 0) return SendClientMessage(playerid, -1, "Gift isn't spawned.");
	if(Winner == 1) return SendClientMessage(playerid, -1, "There is no gift spawn.");
	...
	SendClientMessage(playerid, -1, "You've been teleported near the event.");
	return 1;
}
				...
				for(new i=0;i < MAX_PLAYERS;i++)
			    {
		            if(GetPlayerVirtualWorld(playerid) == cActor[i][World])
		            {
			    		if(GetPlayerInterior(playerid) == cActor[i][Interior])
			        	{
			        		...
			        		{
			        		    if(cActor[i][ActorAmmo] == 1) return 1;
			        		    if(cActor[i][aTeam] == 2) return 1;
			       	    		new string[60];
			       	    		format(string, sizeof(string), ""cwhite"Corpse of: %s", cActor[i][Name]);
			       	    		ShowPlayerDialog(playerid, DIALOG_CMENU, DIALOG_STYLE_LIST, string, ""cwhite"Information\n"cwhite"Plunder corpse", "Select", "Cancel");
			       	    		break;
							}
						}
					}
					    ...
					    for(new i=0;i < MAX_PLAYERS;i++)
                        {
					        if(GetPlayerInterior(playerid) == cActor[i][Interior])
					        {
					        	if(IsPlayerInRangeOfPoint(playerid, 1.0, cActor[i][Pos][0],cActor[i][Pos][1], cActor[i][Pos][2]))
					        	{
					        	    ...
					        	    format(string, sizeof(string), ""cwhite"Corpse of: %s", cActor[i][Name]);
					        	    format(string2, sizeof(string2), ""cwhite""cwhite"> Name: "cgreen"%s\n"cwhite"> The corpse have some ammo.\n", cActor[i][Name]);
					        ...
					        if(GetPlayerInterior(playerid) == cActor[i][Interior])
					        {
					        	if(IsPlayerInRangeOfPoint(playerid, 1.0, cActor[i][Pos][0],cActor[i][Pos][1], cActor[i][Pos][2]))
					        	{
					        	    if(Team[playerid] == HUMAN)
					        	    {
						        	    new string[40];
						        	    format(string, sizeof(string), ""cwhite"Corpse of: %s", cActor[i][Name]);
	                  					ShowPlayerDialog(playerid, DIALOG_CSTEAL, DIALOG_STYLE_MSGBOX, string, ""cwhite"> "cgreen"You want to steal the ammo from the corpse?", "Yes", "No");
									}
									else
									{
									    new string[40];
						        	    format(string, sizeof(string), ""cwhite"Corpse of: %s", cActor[i][Name]);
	                  					ShowPlayerDialog(playerid, DIALOG_CSTEAL, DIALOG_STYLE_MSGBOX, string, ""cwhite"> "cpurple"You want to eat some meat from the corpse?", "Yes", "No");
									}
								}
							}
						}
			...
			case DIALOG_CSTEAL:
			{
   				for(new i=0;i < MAX_PLAYERS;i++)
       			{
		        	if(GetPlayerInterior(playerid) == cActor[i][Interior])
			        {
		        		if(IsPlayerInRangeOfPoint(playerid, 1.0, cActor[i][Pos][0],cActor[i][Pos][1], cActor[i][Pos][2]))
			        	{
			        	    new string[120], str[100];
			        	    if(Team[playerid] == HUMAN)
			        	    {
        						new w[13][2];
								for(new f = 0; f < 13; f++)
								{
								    GetPlayerWeaponData(playerid, f, w[f][0], w[f][1]);
								}
								if(PInfo[playerid][SPerk] == 19) GivePlayerWeapon(playerid,16,3);
								if(w[2][0] == 22) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,22,20);}else{GivePlayerWeapon(playerid,22,50); }
								if(w[2][0] == 23) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,23,20);}else{GivePlayerWeapon(playerid,23,50); }
								if(w[2][0] == 24) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,24,20);}else{GivePlayerWeapon(playerid,24,50); }
								if(w[3][0] == 25) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,25,30);}else{GivePlayerWeapon(playerid,25,20); }
								if(w[3][0] == 26) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,26,30);}else{GivePlayerWeapon(playerid,26,30); }
								if(w[3][0] == 27) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,27,30);}else{GivePlayerWeapon(playerid,27,30); }
								if(w[4][0] == 28) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,28,120);}else{GivePlayerWeapon(playerid,28,120); }
								if(w[4][0] == 32) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,32,120);}else{GivePlayerWeapon(playerid,32,120); }
								if(w[6][0] == 33) if(PInfo[playerid][SPerk] == 12) { GivePlayerWeapon(playerid,33,30);}else{GivePlayerWeapon(playerid,33,30); }

				    			cActor[i][ActorAmmo] = 1;

				    			format(string, sizeof(string), "You have looted the corpse( %s )and you get some ammo.", cActor[i][Name]);
								SendClientMessage(playerid, -1, string);

								format(str,sizeof(str),""cjam"» %s loot some ammo from the corpse.", GetPName(playerid));
								SendNearMessage(playerid, white, str, 30);
							}
							else
							{
				        	    format(string, sizeof(string), "You have eated some meat from the corpse( %s ).", cActor[i][Name]);
								SendClientMessage(playerid, -1, string);

								format(str,sizeof(str),""cjam"» %s eat meat from the corpse.",GetPName(playerid));
								SendNearMessage(playerid,white,str,30);

							    new Float:zhp;
							    GetPlayerHealth(playerid, zhp);
							    ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.0, 0, 0, 0, 0, 0);
							    SetPlayerHealth(playerid, zhp+35);
							    if(zhp > 100) SetPlayerHealth(playerid, 100);
							    cActor[i][ActorAmmo] = 1;
							}
						}
					}
	...
	else
	{
	    switch(dialogid)
	    {
         	case DIALOG_CINFO:
	        {
			    ...
			    {
		            if(GetPlayerVirtualWorld(playerid) == cActor[x][World])
		            {
			    		if(GetPlayerInterior(playerid) == cActor[x][Interior])
			        	{
			        		if(IsPlayerInRangeOfPoint(playerid, 1.0, cActor[x][Pos][0],cActor[x][Pos][1], cActor[x][Pos][2]))
			        		{
			        		    new stri[60];
			        		    if(Team[playerid] == HUMAN)
			        		    {
				       	    		format(stri, sizeof(stri), ""cwhite"Corpse of: %s", cActor[x][Name]);
				       	    		ShowPlayerDialog(playerid, DIALOG_CMENU, DIALOG_STYLE_LIST, str, ""cwhite"Information\n"cwhite"Plunder corpse", "Select", "Cancel");
								} else {
				       	    		format(stri, sizeof(stri), ""cwhite"Corpse of: %s", cActor[x][Name]);
				       	    		ShowPlayerDialog(playerid, DIALOG_CMENU, DIALOG_STYLE_LIST, str, ""cwhite"Information\n"cwhite"Eat meat", "Select", "Cancel");
								}
...
function TikiStatueFix()
{
	foreach(new i:Player)
	{
		...
		if(IsPlayerInRangeOfPoint(i, 2.5, PX, PY, PZ))
		{
			...
			gettime(Hour, Minute, Second);
			SendFMessageToAll(-1, "» {85C1E9}A new gift will be hidden in %d minutes.", Minutes-Minute+10);
...
stock DamagePlayer(playerid,i)
{
	...
	if(Health <= 5)
	{
	    /*if(!strcmp(cActor[i][Name], ""))
		{
		    DestroyActor(cActor[i][ActorID]);
			Delete3DTextLabel(cActor[i][LabelID]);
		}*/
		new str[100];
		GetPlayerName(i, cActor[i][Name], 24);
		GetPlayerPos(i, cActor[i][Pos][0],cActor[i][Pos][1], cActor[i][Pos][2]);
		GetPlayerFacingAngle(playerid, cActor[i][Pos][3]);
		cActor[i][SkinID] = GetPlayerSkin(i);
		cActor[i][Interior] = GetPlayerInterior(i);
		cActor[i][World] = GetPlayerVirtualWorld(i);
		cActor[i][aTeam] = 1;

		cActor[i][ActorID] = CreateActor(cActor[i][SkinID],cActor[i][Pos][0],cActor[i][Pos][1],cActor[i][Pos][2],cActor[i][Pos][3]);
		SetActorHealth(cActor[i][ActorID], 0);
	 	ApplyActorAnimation(cActor[playerid][ActorID], "PED", "FLOOR_hit_f", 4.1, 0, 1, 1, 1, 0);
	 	format(str, sizeof(str), ""cwhite"Corpse: "cgreen"%s "cwhite"| Team: "cgreen"Human", cActor[playerid][Name]);
		cActor[i][LabelID] = Create3DTextLabel(str, 0xFFFFFFFF, cActor[i][Pos][0], cActor[i][Pos][1], cActor[i][Pos][2], 10.0, cActor[i][World], 1);
		SetTimerEx("KillActor", 300000, 0, "d", i);
...
function TikiEvent()
{
	...
	SendClientMessageToAll(0x2874A6FF, "[ » ] Christmas Event [ « ]");
	format(string, sizeof(string), "» {85C1E9}A new gift was hidding in the area: %s.", LocationsName[rand]);
	SendClientMessageToAll(-1, string);
	SendClientMessageToAll(-1, "» {85C1E9}You have 10 minutes to find this gift.");
