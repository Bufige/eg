...
public OnGameModeInit()
{
    ...
    TD_INTRO[0] = TextDrawCreate(643.000000, 2.000000, "_");
	TextDrawBackgroundColor(TD_INTRO[0], 255);
	TextDrawFont(TD_INTRO[0], 1);
	TextDrawLetterSize(TD_INTRO[0], 0.500000, 3.499999);
	TextDrawColor(TD_INTRO[0], -1);
	TextDrawSetOutline(TD_INTRO[0], 0);
	TextDrawSetProportional(TD_INTRO[0], 1);
	TextDrawSetShadow(TD_INTRO[0], 1);
	TextDrawUseBox(TD_INTRO[0], 1);
	TextDrawBoxColor(TD_INTRO[0], 90);
	TextDrawTextSize(TD_INTRO[0], -4.000000, 0.000000);

	TD_INTRO[1] = TextDrawCreate(643.000000, 382.000000, "_");
	TextDrawBackgroundColor(TD_INTRO[1], 255);
	TextDrawFont(TD_INTRO[1], 1);
	TextDrawLetterSize(TD_INTRO[1], 0.500000, 6.999999);
	TextDrawColor(TD_INTRO[1], -1);
	TextDrawSetOutline(TD_INTRO[1], 0);
	TextDrawSetProportional(TD_INTRO[1], 1);
	TextDrawSetShadow(TD_INTRO[1], 1);
	TextDrawUseBox(TD_INTRO[1], 1);
	TextDrawBoxColor(TD_INTRO[1], 150);
	TextDrawTextSize(TD_INTRO[1], -4.000000, 0.000000);

	TD_INTRO[2] = TextDrawCreate(249.000000, 29.000000, "eternal-Games.net");
	TextDrawBackgroundColor(TD_INTRO[2], 255);
	TextDrawFont(TD_INTRO[2], 1);
	TextDrawLetterSize(TD_INTRO[2], 0.500000, 1.000000);
	TextDrawColor(TD_INTRO[2], -1);
	TextDrawSetOutline(TD_INTRO[2], 1);
	TextDrawSetProportional(TD_INTRO[2], 1);

	TD_INTRO[3] = TextDrawCreate(93.000000, 8.000000, "] ZOMBIE APOCALYPTIC OUTBREAK ]");
	TextDrawBackgroundColor(TD_INTRO[3], 255);
	TextDrawFont(TD_INTRO[3], 2);
	TextDrawLetterSize(TD_INTRO[3], 0.579999, 1.800000);
	TextDrawColor(TD_INTRO[3], -1);
...
public OnPlayerRequestClass(playerid, classid)
{
	...
	if(PInfo[playerid][P_STATUS] == PS_CONNECTED)
	{
		...
		Streamer_UpdateEx(playerid, 2402.475341, -1709.786010, 14.132812);
		SetPlayerPos(playerid, 2402.475341, -1709.786010, 14.132812);
		SetPlayerFacingAngle(playerid, 170.784210);
		SetPlayerCameraPos(playerid, 2402.464355, -1713.951782, 14.933802);
		SetPlayerCameraLookAt(playerid,2402.475341, -1709.786010, 14.132812);
	...
	{
	    Team[playerid] = HUMAN,GameTextForPlayer(playerid,"~n~~n~~n~~n~~n~~n~~n~~n~~p~Survivor",3000,3);
	}
	else if(classid >= 0 && classid <= 11)
	{
	    Team[playerid] = ZOMBIE,GameTextForPlayer(playerid,"~n~~n~~n~~n~~n~~n~~n~~n~~g~~h~Zombie",3000,3);
	}
	return 1;
}
	...
	if(Team[playerid] == HUMAN)
	{
		if(IsSpecing[playerid] == 1)
		{
			...
			if(PInfo[playerid][SSkin] != 0 && PInfo[playerid][Premium] > 0)	{
				...
				SetPlayerSkin(playerid, HumansSkins[random(sizeof(HumansSkins))]); }
			return 1;
		}
...
/*	for(new i; i < MAX_PLAYERS;i++)
	...
	if(db_num_rows(Result))
	{
		...
		if(PInfo[playerid][Banned])
		{
			...
			format(file,sizeof file2,"%s has tried to ban evade.",GetPName(playerid));
			SaveIn("Banevadelog", file2, 1);
...
ServerPickUps()
{
	...
	ZTPS[4] = CreateDynamicPickup(1318, 1, 34.202831, 1486.785034, 12.800000, -1);
 	...
 	TPZone[10] = CreateDynamicPickup(1318, 1, 975.114929, -1525.581298, 13.559997, -1); // Market
 	TPZone[11] = CreateDynamicPickup(1318, 1, 237.843780, -1387.799804, 53.594272, -1); // Mansion
